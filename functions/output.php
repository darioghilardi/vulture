...
**/
	...
	function getVulnNodeTitle($func_name)
	{
		...
		{	$vulnname = $GLOBALS['NAME_POP']; $GLOBALS['count_pop']++; }
		else if(isset($GLOBALS['F_OTHER'][$func_name])) 
		{	$vulnname = $GLOBALS['NAME_OTHER']; $GLOBALS['count_other']++; } // :X			 			
		else 
			...
			{				
				...
				{				
					...
					{		
						...
						addslashes($tree->filename ? $tree->filename : key($output)), '\',\'',
	...
	function createFileList($files, $file_sinks)
	{
		if(!empty($files))
		{
			...
			// get vuln files
			$vulnfiles = array();
			foreach($GLOBALS['output'] as $filename => $tree)
			{		
				$vulnfiles[] = realpath($filename);
			}	
	...
	{
		echo '<tr><td nowrap>',$name,':</td><td nowrap><div id="chart'.$nr.'" class="chart" style="width:',
